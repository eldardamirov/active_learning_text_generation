...
def parse_args(args):
    ...
    parser.add_argument(
        "--mode",
        default="hm-correlation",
        choices=["hm-correlation", "ablations", "ablations-plot", "mm-correlation"],
        help=(
    ...
    parser.add_argument(
        "--human_eval_path",
        default="/home/phillab/data/frank/human_annotations.json",
        help="file containing human annotations expects csv file.",
    )
    parser.add_argument(
        "--baseline_metrics_outputs",
        default="/home/phillab/data/frank/baseline_factuality_metrics_outputs.json",
        help="file name containing outputs of baseline factuality metrics.",
    )
    parser.add_argument(
        "--baseline_metrics",
        nargs="+",
        default=baseline_metrics,
        help="baseline metrics to evaluate on (should match the name in the baseline metrics output file).",
    )
    parser.add_argument(
        "--no_baseline_metrics",
        action="store_true",
        help="If set, does not evaluate the baseline metrics",
    )
    parser.add_argument(
        "--metrics_outputs",
        default=None,
        help='names of json files containing metric outputs with key "score"',
    )
    parser.add_argument(
        "--metrics_outputs_info",
        default=None,
        help="json file describing how to parse metrics output files. This allows to customize the name of the score key and to have several metrics in one json file.",
    )
    parser.add_argument(
        "--ablations",
        nargs="+",
        default=ablations_cols,
        help="column names for ablations.",
    )
    parser.add_argument(
        "--human", default="Factuality", help="column for human judgements."
    )
    parser.add_argument("--no_partial_correlation", action="store_true")
    parser.add_argument(
        "--partial_correlation_variable",
        default="model_name",
        help="what column to use as confounding to calculate partial correlations",
    )
    parser.add_argument("--store_path", default=None)
    parser.add_argument(
        "--dataset",
        default=None,
        choices=[None, "cnndm", "bbc"],
        help="if None use all data",
    )
    parser.add_argument(
        "--model_name",
        nargs="+",
        default=None,
        help=f"by default use all data, availble model names {model_names}",
    )
...
def human_metric_correlation(
    ...
    for metric in metrics_cols:
        if metric not in data_df:
            ...
            continue
        mask = (data_df[metric].isnull() == False) & (
            data_df[human_col].isnull() == False
        )
        X = data_df[metric][mask]
        Y = data_df[human_col][mask]
        if partial_correlation:
            assert (
                partial_correlation_variable is not None
            ), f"You must specify a column to use as confounding variable for partial correlation calculation"
            ...
            Y = Y - pred_Y
        print(
            f"Info: metric {metric} used {len(X)} summaries to calculate correlation."
        )
        pr, pp = pearsonr(X, Y)
        sr, sp = spearmanr(X, Y)
...
def metric_metric_correlation(
    ...
    for i, metric1 in enumerate(metrics_cols):
        ...
        for j, metric2 in enumerate(metrics_cols):
            if j == i:
                ...
                continue
            mask1 = (data_df[metric1].isnull() == False) & (
                data_df["model_name"] != "reference"
            )
            mask2 = (data_df[metric2].isnull() == False) & (
                data_df["model_name"] != "reference"
            )
    ...
    correlations_df = pd.DataFrame(
        correlations, index=metrics_cols, columns=metrics_cols
    )
...
def ablation(
    ...
    human_df = human_metric_correlation(
        data_df,
        human_col,
        metrics_cols,
    ...
    for ablation in ablations_cols:
        ablation_df = human_metric_correlation(
            data_df,
            ablation,
            metrics_cols,
            partial_correlation=partial_correlation,
...
def main(args):
    ...
    if not args["no_baseline_metrics"]:
        ...
        for baseline_metric in args["baseline_metrics"]:
            assert baseline_metric in metric_df, (
                baseline_metric
                + " not found. Your metrics_output_info file is likely not well defined."
            )
        data_df = data_df.merge(
            metric_df[["hash", "model_name"] + args["baseline_metrics"]],
            on=["hash", "model_name"],
            validate="one_to_one",
        )
    ...
    if args["metrics_outputs"]:
        ...
        assert (
            "score" in metric_df
        ), 'The metric output should be in a field named "score"'
        data_df = data_df.merge(
            metric_df[["hash", "model_name", "score"]],
            on=["hash", "model_name"],
            validate="one_to_one",
        )
        metrics_cols += ["score"]
    ...
    if args["metrics_outputs_info"]:
        ...
        for metric_info in metrics_info:
            ...
            for score_info in metric_info["scores"]:
                assert score_info["key"] in metric_df, (
                    score_info["key"]
                    + " not found. Your metrics_output_info file is likely not well defined."
                )
                keys.append(score_info["key"])
            data_df = data_df.merge(
                metric_df[["hash", "model_name"] + keys],
                on=["hash", "model_name"],
                validate="one_to_one",
            )
            data_df = data_df.rename(
                columns={
                    score_info["key"]: score_info["name"]
                    for score_info in metric_info["scores"]
                }
            )
    ...
    if args["dataset"]:
        mask = (data_df["dataset"] == args["dataset"]) & (
            data_df["model_name"] != "reference"
        )
        data_df = data_df[mask]
    if args["model_name"]:
        mask = (data_df["model_name"].isin(args["model_name"])) & (
            data_df["model_name"] != "reference"
        )
        data_df = data_df[mask]
    ...
    if args["store_path"]:
        ...
        if williams_df is not None:
            williams_df.to_json(
                os.path.join(args["store_path"], "williams.json"), indent=4
            )
